...
void permashow_on_draw()
{
	...
	for (auto& element : permashow::instance.permashow_elements)
	{
		...
		draw_manager->add_text_on_screen(position + element.value_pos + offset_from_top,
		                                 MAKE_COLOR(static_cast<int>(element.value_color.x), static_cast<int>(element.value_color.y), static_cast<int>(element.value_color.z), ((text_color->get_color()) >> 24) & 0xff),
...
void permashow::init(TreeTab* others_menu)
{
	...
	const auto permashow = others_menu->add_tab("permashow", "Permashow");
	{
		...
		permashow_pos_y = permashow->add_slider("posy", "Position Y", static_cast<int>(renderer->screen_height() * 0.74f), 0, renderer->screen_height() - 50.f);
